...
private:
    string name;
    list<string> arg_values;
    //Added sets for storing grounded effects and grounded preconditions
...
public:
    ...
    //adds a grounded precondition into the set
    void add_grounded_precondition(GroundedCondition &gc)
    {
        this->groundedPreconditions.insert(gc);
    }

    //adds a grounded effect into the set
    void add_grounded_effect(GroundedCondition &gc)
    {
        this->groundedEffects.insert(gc);
    }
...
typedef unordered_set<GroundedCondition, GroundedConditionHasher, GroundedConditionComparator> GState;


//A state is created based on a set of grounded conditions
class State
{
    public:
    int f_val = 0;
    int g_val = 0;
    int h_val = 0;
    GState grounded_conditions;
    State* parent; //The parent of the state
    GroundedAction parentAction;    //The action that led to the state

    State() {}

    State(GState grounded_conditions)
    {
        this->grounded_conditions = grounded_conditions;
    }
...
{
    bool operator()(State* c1, State* c2)
    {
...
class SymPlan
{
    ...
    list<GroundedAction> allGroundedActions; //stores all grounded actions
    unordered_set<string> symbols;
    unordered_set<Action, ActionHasher, ActionComparator> Actions;
    list<list<string>> permutations; //stores permutations of a certain number of symbols
    list<GroundedAction> path; //A-star path of grounded actions
    ...
    //This function recursively calculates the different permutations of symbols
    void Permute(int n, list<string> temp, list<list<string>> &perm, unordered_set<string> syms)
    {
        if(temp.size() == n)
        {
            perm.push_back(temp);
        ...
        else
        {
            if(temp.size())
            {
                syms.erase(temp.back());
            }
            for (auto i : syms)
            {
                temp.push_back(i);
                Permute(n, temp, perm, syms);
                temp.pop_back();
            }
        }
        return;
    ...
    //This generates the permuations for a certain number of symbols n
    list<list<string>> Permutations(int n, unordered_set<string> syms)
    {
        list<string> temp_perm;
        list<list<string>> perm;
        Permute(n, temp_perm, perm, syms);
        return perm;

    }

    //Generates all the permuations of the symbols up till the maximum number of symbols in an action
    //and stores it in a the output data structure
    vector<list<list<string>>> GeneratePermutations(unordered_set<string> syms)
    {
        ...
        {
            all_permutations.push_back(Permutations(i, syms));
        }
        return all_permutations;
    }

    //This function calculates and stores all the possible grounded actions in a list
    void AllGroundedActions()
    {
        unordered_set<string> syms = symbols;
        ...
        for (auto &action: Actions)
        {
            ...
            for(auto &gargs: permutations[no_args])
            {
                GroundedAction g_action(action.get_name(), gargs);
                unordered_map<string, string> ArgsMap; //Argument map between the arguments of the action and symbols

                for(auto &symbol: symbols)
                {
                    ArgsMap[symbol] = symbol;
                }
                auto it = gargs.begin();
                for (auto &arg: args)
                {
                ...
                {
                    list<string> precondition_args;
                    for(const auto &precondition_arg: precondition.get_args())
                    {
                        precondition_args.push_back(ArgsMap[precondition_arg]);
                    }
                    GroundedCondition g_condition(precondition.get_predicate(), precondition_args, precondition.get_truth());
                    g_action.add_grounded_precondition(g_condition);
                }
                for(Condition effect: effects)
                {
                    list<string> effect_args;
                    for(const auto &effect_arg: effect.get_args())
                    {
                        effect_args.push_back(ArgsMap[effect_arg]);
                    }
                    GroundedCondition g_condition(effect.get_predicate(), effect_args, effect.get_truth());
                    g_action.add_grounded_effect(g_condition);
                                
                }
                allGroundedActions.push_back(g_action);
            }
        }   
    }

    //Outputs the set of grounded conditions after applying an action to a state
    GState GetSuccesor(GroundedAction &action, State* &state)
    {
        GState newStateConds = state->grounded_conditions;

        for(auto effect: action.get_effects())
        {
            if(effect.get_truth())
            {
                newStateConds.insert(effect);
            }
            else
            {
                // effect.set_truth(true);
                effect.flip_bool();
                newStateConds.erase(effect);
            }
        }

        return newStateConds;
    }
    
    //Check if the action that will be applied satisfies the preconditions
    bool ValidAction(GroundedAction &action, State* &state)
    {
        for(auto precond: action.get_preconditions())
        {
            if(state->grounded_conditions.find(precond) == state->grounded_conditions.end())
            {
                return false;
            }
        }
    ...
    //Compares the sets of grounded conditions within a state
    bool CompareState(State* state1, State* state2)
    {
        for(GroundedCondition gc: state2->grounded_conditions)
        {
            if(state1->grounded_conditions.find(gc) == state1->grounded_conditions.end())
            {
                return false;
            }
        }
    ...
    //Compares two sets of grounded conditions
    ...
    //Simple Heuristic based on the the number of conditions in the goal state that do not exist in the current state
    int Heuristic(State* state, State* goal)
    {
        int h = 0;
        for(GroundedCondition gc: goal->grounded_conditions)
        {
            if(state->grounded_conditions.find(gc) == state->grounded_conditions.end())
            {
                h++;
            }
        }
    ...
    //This function checks if the state with a set of grounded conditions exists in the Closed list
    ...
    //Regular A-star search
    void AStar()
    {
        cout << "AStar" << endl;

        //Priority Queue storing a pointer to the State, that compares based on the f value
        priority_queue<State*, vector<State*>, f_value_compare> OPEN;
        //Unordered Set of State pointers in the closed list
        unordered_set<State*> CLOSED;

        State* goalState = new State(env->get_goal_conditions());

        State* start = new State(env->get_initial_conditions());
        start->g_val = 0;
        start->h_val = Heuristic(start, goalState);
        // start->h_val = 0;
        start->f_val = 0;
        start->parent = 0;
        ...
        while(!OPEN.empty())
        {
            ...
            if(StateExists(best->grounded_conditions, CLOSED)) continue;
            ...
            for(auto action: allGroundedActions)
            {
                if(ValidAction(action, best))
                {
                    GState newStateCond = GetSuccesor(action, best);

                    //If the state doesnt already exist in the closed list
                    if(!StateExists(newStateCond, CLOSED))
                    {
        ...
        while(current_state->parent != start->parent)
        {
            path.push_back(current_state->parentAction);
