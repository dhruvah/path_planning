...
void Search::astar(){
    ...
    point2d_int goal_point = getGoal();

    // goal_point.x = goalposeX;
    // goal_point.y = goalposeY;
    ...
    while(!OPEN.empty()) {
        ...
        if ((best.location.x == goal_point.x) && (best.location.y == goal_point.y)){
        ...
        for (auto &neighbor : getNeighbors(best.location))
        {
            ...
            int MapValue = getMapValue(n_row, n_col);
            if(MapValue >= 0 && MapValue <= collision_thresh){
                int new_cost = best.g_value + MapValue;
                ...
                if (neighbor.g_value > new_cost)
                {
                    ...
                    neighbor.f_value = neighbor.g_value + 100*getHeuristic(n_row, n_col);
        ...
        )
{
    ...
    cout << "Goal kmkbc:  " << goalposeX << " " << goalposeY << endl;
